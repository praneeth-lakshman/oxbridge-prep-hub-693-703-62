      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSignUp)} className="space-y-4">
            ...
            {/* Stripe Connect Setup for Tutors */}
            {selectedUserType === 'tutor' && (
              <div className="space-y-4 animate-fade-in">
                <Alert className="border-amber-200 bg-amber-50">
                  <AlertTriangle className="h-4 w-4 text-amber-600" />
                  <AlertDescription className="text-amber-800">
                    <span className="font-medium">Payment Setup Required:</span> You won't be able to receive payments from students without connecting your Stripe account. You can skip this now and set it up later in your profile section.
                  </AlertDescription>
                </Alert>
                
                <div className="bg-secondary/30 p-4 rounded-lg border">
                  <h3 className="text-sm font-medium mb-3">Set Up Payments (Recommended)</h3>
                  <StripeConnectOnboarding onComplete={() => {
                    toast({
                      title: "Payment Setup Complete!",
                      description: "You can now receive payments from students.",
                    });
                  }} />
                </div>
              </div>
